<template>
  <button
    ...
    :title="exporting ? $t('shared.preparingExport') : displayTitle"
    @click="$emit('click')"
  >
    ...
    <span>{{ exporting ? $t('common.exporting') : displayLabel }}</span>
  </button>
</template>
...
const displayLabel = computed(() => props.label || t('common.export'))
const displayTitle = computed(() => props.title || t('shared.exportToExcelTitle'))
</script>
