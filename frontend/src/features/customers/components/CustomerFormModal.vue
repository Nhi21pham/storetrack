  ...
  <div class="modal-overlay" @click.self="handleClickOutside">
    <div class="modal">
      <div class="modal-header">
        <h2>{{ isEdit ? $t('customers.editCustomer') : $t('customers.newCustomer') }}</h2>
      ...
      <div class="modal-body">
        <div class="form-group">
          <label>{{ $t('customers.customerName') }} <span class="required">*</span></label>
          <input v-model="form.name" type="text" :placeholder="$t('customers.namePlaceholder')" :class="{ error: errors.name }" />
          <span v-if="errors.name" class="error-text">{{ errors.name }}</span>
        </div>

        <div class="form-group">
          <label>{{ $t('customers.taxCode') }}</label>
          <input v-model="form.tax_code" type="text" :placeholder="$t('customers.taxCodePlaceholder')" :class="{ error: errors.tax_code }" />
          <span v-if="errors.tax_code" class="error-text">{{ errors.tax_code }}</span>
        </div>

        <div class="form-group">
          <label>{{ $t('customers.address') }}</label>
          <input v-model="form.address" type="text" :placeholder="$t('customers.addressPlaceholder')" :class="{ error: errors.address }" />
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <input v-model="form.email" type="email" :placeholder="$t('customers.emailPlaceholder')" :class="{ error: errors.email }" />
            <span v-if="errors.email" class="error-text">{{ errors.email }}</span>
          </div>
          <div class="form-group">
            <label>{{ $t('customers.phone') }} <span class="required">*</span></label>
            <input v-model="form.phone" type="tel" :placeholder="$t('customers.phonePlaceholder')" :class="{ error: errors.phone }" />
        ...
        <BankAccountsFormWidget
          ...
          :party-id="customer?.party?.id ?? null"
          :default-holder-name="form.name"
          :entity-label="$t('customers.entityLabel')"
        />
      </div>
      ...
      <div class="modal-footer">
        ...
        <button class="btn-submit" @click="handleSubmit" :disabled="loading || !isDirty">
          ...
          {{ isEdit ? $t('common.saveChanges') : $t('customers.createCustomer') }}
        </button>
      </div>
    </div>
  </div>
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const partyId = result?.party?.id ?? props.customer?.party?.id ?? props.customer?.party_id
