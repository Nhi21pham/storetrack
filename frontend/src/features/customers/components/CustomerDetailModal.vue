    ...
    <div class="modal">
      ...
      <div class="modal-body">
        <div class="detail-row">
          <span class="detail-label">{{ $t('customers.idLabel') }}</span>
          <span class="detail-value mono">#{{ customer.id }}</span>
        </div>

        <div class="detail-row">
          <span class="detail-label">{{ $t('customers.name') }}</span>
          <span class="detail-value name-text">{{ customer.name }}</span>
        </div>

        <div class="detail-row">
          <span class="detail-label">{{ $t('customers.taxCode') }}</span>
          <span v-if="customer.tax_code" class="detail-value mono">{{ customer.tax_code }}</span>
          <span v-else class="detail-value empty-val">—</span>
        </div>

        <div class="detail-row">
          <span class="detail-label">{{ $t('customers.email') }}</span>
          <span v-if="customer.email" class="detail-value">{{ customer.email }}</span>
          <span v-else class="detail-value empty-val">—</span>
        </div>

        <div class="detail-row">
          <span class="detail-label">{{ $t('customers.phone') }}</span>
          <span v-if="customer.phone" class="detail-value mono">{{ customer.phone }}</span>
          <span v-else class="detail-value empty-val">—</span>
        </div>
      ...
      <div class="modal-footer">
        ...
        <button v-if="canEdit" class="btn-edit" @click="$emit('edit', customer)">
