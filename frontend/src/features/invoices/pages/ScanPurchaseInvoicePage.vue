<template>
  <PageContainer :maxWidth="1100">
    <PageHeader :title="$t('invoices.scanPurchaseTitle')" :subtitle="$t('invoices.scanSubtitle')">
      <template #actions>
        ...
        <section class="card">
          ...
          <div class="details-grid">
            <div class="form-group">
              ...
              <span v-if="extractedSupplierLabel" class="hint">{{ $t('invoices.readLabel') }} {{ extractedSupplierLabel }}</span>
            </div>
...
const bannerGroups = computed(() => {
  ...
  if (ex?.name && !supplierPartyId.value) {
    groups.push({ type: 'supplier', label: t('invoices.suppliers'), chips: [{ id: 'supplier', label: ex.name }] })
  }
  const seen = new Map()
