<template>
  <div class="suggestions" @mousedown.prevent>
    <div class="suggestions-header">{{ $t('productCategories.existingMatches') }}</div>
    <button
      v-for="item in items"
      :key="item.id"
    ...
    >
      <div class="row-main">
        ...
        <span class="name">{{ displayCategoryName(item) }}</span>
        <span v-if="item.is_system" class="system-badge">{{ $t('productCategories.systemBadge') }}</span>
...
.row-main { display: flex; align-items: center; gap: 8px; font-size: 14px; color: #111; }
.code { font-family: ui-monospace, SFMono-Regular, Menlo, monospace; font-weight: 600; color: #4338ca; }
.name { font-weight: 500; }
.system-badge { font-size: 10px; text-transform: uppercase; letter-spacing: 0.05em; padding: 2px 6px; border-radius: 4px; background: #fae8ff; color: #a21caf; font-weight: 600; }
