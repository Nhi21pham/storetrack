<template>
  <PageContainer :maxWidth="1280">
    <PageHeader title="Profit Report" subtitle="Profit per batch sold, one row per batch-slice of a sale." />

    <EmptyState
      v-if="scope === 'store' && !currentStore"
      title="No store selected"
      description="Select a store to view its profit report."
    />

    <EmptyState
      v-else-if="scope === 'business' && !currentBusiness"
      title="No business selected"
      description="Select a business to view its consolidated profit report."
    />

    <template v-else>
      <div class="toolbar">
        <SearchBar v-model="searchQuery" placeholder="Search product or invoice..." />
      ...
      <LoadingState v-if="loading">Loading profit report…</LoadingState>

      <EmptyState
        v-else-if="rows.length === 0"
        title="No sales yet"
        description="Record a sale invoice to start tracking profit per batch."
      />
    ...
    <HistoryModal
      v-if="showHistory"
      title="Profit Report — Export History"
      :tabs="historyTabs"
...
const { exporting, run } = useExport({
  ...
  onSuccess: () => showToast('Profit report export ready.', 'success'),
  onError:   (msg) => showToast(msg, 'error'),
})
