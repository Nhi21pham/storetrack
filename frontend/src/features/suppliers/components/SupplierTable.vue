    ...
    <tr v-for="(supplier, idx) in suppliers" :key="supplier.id" :class="{ 'row-selected': isSelected(supplier.id) }">
      ...
      <td class="actions-col">
        <div v-if="rowActionsEnabled && canManageRow(supplier)" class="row-actions">
          <button class="action-btn" @click="$emit('edit', supplier)" :title="$t('common.edit')">
            <Icon name="edit" :size="14" />
          </button>
          <button v-if="canDelete" class="action-btn danger" @click="$emit('delete', supplier)" :title="$t('common.delete')">
