  ...
  <div class="modal-overlay" @click.self="handleClickOutside">
    <div class="modal">
      <div class="modal-header">
        <h2>{{ isEdit ? $t('products.editProduct') : $t('products.newProduct') }}</h2>
      ...
      <div class="modal-body">
        <div v-if="isEdit" class="form-group">
          ...
          <input
            ...
            class="code-display"
          />
          <p class="hint">{{ $t('products.autoGenerated') }}</p>
        </div>

        <div class="form-group">
          <label>{{ $t('products.category') }} <span class="required">*</span></label>
          <div class="picker-row">
            ...
            >
              <option value="" disabled>{{ categoriesLoading ? $t('products.loadingCategories') : $t('products.selectCategory') }}</option>
            ...
            <AddItemButton
              v-if="!isEdit"
              size="small"
              :title="$t('products.createCategoryTitle')"
          ...
          <p v-if="!isEdit && form.product_category_id && nextCodePreview" class="hint">
            {{ $t('products.nextCode') }} <strong>{{ nextCodePreview }}</strong>
          </p>
          <p v-else-if="!isEdit" class="hint">
            {{ $t('products.codeAutoHint') }}
          </p>
          <p v-if="isEdit" class="hint">{{ $t('products.categoryLocked') }}</p>
        </div>

        <div class="form-group" :class="{ 'has-suggestions': showNameSuggestions }">
          <label>{{ $t('products.productName') }} <span class="required">*</span></label>
          <input
            v-model="form.name"
            type="text"
        ...
        <div class="form-group">
          <label>{{ $t('products.unit') }} <span class="required">*</span></label>
          <div class="picker-row">
            <SelectField
            ...
            >
              <option value="" disabled>{{ unitsLoading ? $t('products.loadingUnits') : $t('products.selectUnit') }}</option>
              <option v-for="u in activeUnits" :key="u.id" :value="String(u.id)">{{ u.name }}</option>
            </SelectField>
            <AddItemButton
              size="small"
              :title="$t('products.createUnitTitle')"
              :disabled="unitsLoading"
        ...
        <div v-if="isEdit" class="form-group toggle-group">
          <div class="toggle-row">
            ...
            <span class="toggle-text">{{ $t('products.activeToggle') }}</span>
          </div>
          <p class="hint">{{ $t('products.inactiveHint') }}</p>
        </div>
...
const validateForm = () => {
  ...
  if (!name) errors.value.name = t('products.nameRequired')
  else if (name.length > 255) errors.value.name = t('products.nameTooLong')

  if (!form.value.unit_id) errors.value.unit_id = t('products.unitRequired')
  if (!isEdit.value && !form.value.product_category_id) {
    errors.value.product_category_id = t('products.categoryRequired')
  }
  ...
  if (!isEdit.value) {
    ...
    if (dup) {
      apiError.value = t('products.duplicateName', { name: dup.name })
      return false
    }
  }
