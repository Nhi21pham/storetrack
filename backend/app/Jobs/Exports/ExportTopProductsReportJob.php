...
/**
 * Generates a Top Products Excel export. The Export record carries the scope (a
 * single store, or every store in a business) and filter context in metadata;
 * the aggregated query is rebuilt here to avoid serializing the whole builder.
 */
class ExportTopProductsReportJob extends BaseExportJob
{
    public const TYPE_STORE = 'top-products-report';

    public const TYPE_BUSINESS = 'top-products-report-business';
    ...
    protected function buildExport(Export $export): BaseExport
    {
        ...
        $repository = app(TopProductsReportRepository::class);

        if ($isBusinessView) {
            $scopeName = $metadata['scope_name'] ?? (Business::find($scopeId)?->name ?? '');
            $title = __('exports.of_business', ['label' => __('exports.label_top_products'), 'name' => $scopeName]);
            $query = $repository->listQueryForBusiness($scopeId, $filters);
        } else {
            $scopeName = $metadata['scope_name'] ?? (Store::find($scopeId)?->name ?? '');
            $title = __('exports.of_store', ['label' => __('exports.label_top_products'), 'name' => $scopeName]);
        ...
        return new TopProductsReportExport($query, $title, $metaLines, $columns);
    }
    ...
    protected function filename(Export $export): string
    {
        ...
        return "top-products-report-{$slug}-{$datetime}.xlsx";
    }
    ...
    protected function onCompleted(Export $export): void
    {
        ...
        if ($export->type === self::TYPE_BUSINESS) {
            app(ReportAuditLogger::class)->reportExportedForBusiness($user, $scopeId, $scopeName, $export, 'top-products');
        } else {
            app(ReportAuditLogger::class)->reportExported($user, $scopeId, $export, $scopeName, 'top-products');
        }
    }
}
